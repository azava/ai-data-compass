...
Pull requests run the test suite and security audit. Pushes to `homolog` publish development builds to TestPyPI through `.github/workflows/publish-to-testpypi.yml`. The workflow reads the development release base from `tool.ai-data-compass.release.testpypi-base-version` in `pyproject.toml` and creates a unique `.devN` version in its temporary runner checkout, where `N` is the GitHub Actions workflow run number. It synchronizes package metadata and the CLI-reported version in that temporary checkout; committed source files are unchanged. After publishing, a separate job installs that exact version from TestPyPI in a clean virtual environment, using PyPI for dependencies, then checks package metadata, import, and the CLI's version and help commands. It retries installation while the package index propagates the release. The TestPyPI publisher uses the separate `testpypi` GitHub environment and never publishes these builds to PyPI; that environment must allow deployments from `homolog`.
