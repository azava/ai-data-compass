...
The package version is declared in `pyproject.toml`. Before preparing a release, compare it with the latest version published on PyPI and verify the current release configuration. To install the current local source, run these commands from the repository root:
...
Stable releases are published to PyPI by `.github/workflows/publish-to-pypi.yml` when a stable `vMAJOR.MINOR.PATCH` tag is pushed. The workflow fetches the full history of `main` to verify that the tag points to a commit contained in it, while avoiding unrelated branches and tags. It also verifies that the tag matches the project version, builds and validates a wheel and source distribution, then publishes through the protected `pypi` GitHub environment using Trusted Publishing. After publishing, a separate job installs that exact version from PyPI in a clean virtual environment and checks package metadata, import, and the CLI's version and help commands. It retries installation while the package index propagates the release.

Pull requests run the test suite and security audit. The release flow advances one stable version through `dev`, `homolog`, and `main`: pushes to `dev` publish `<project.version>.devN` to TestPyPI, where `N` is the GitHub Actions workflow run number; pushes to `homolog` publish the exact stable `project.version` from `pyproject.toml` to TestPyPI; and a matching stable tag on `main` publishes that version to PyPI. The TestPyPI workflow updates package metadata and the CLI-reported version only in its temporary runner checkout, leaving committed source files unchanged. Both `dev` and `homolog` runs install the exact published TestPyPI version in a clean virtual environment, using PyPI for dependencies, then check package metadata, import, and the CLI's version and help commands. The PyPI workflow runs the corresponding smoke test against PyPI. These post-publication checks retry installation while the package index propagates each release. The TestPyPI publisher uses the separate `testpypi` GitHub environment; that environment must allow deployments from both `dev` and `homolog`. TestPyPI does not allow replacing an already published version, so publish a given stable version from `homolog` only once; increment `project.version` when a new homolog candidate is needed.
