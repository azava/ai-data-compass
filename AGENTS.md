...
- Review and correct clear documentation issues across the repository, including README files, instructions, templates, backlog documents, and integration documentation. Use the documentation-validation procedure configured for the current project. If no local procedure exists, look under `.ai-data-compass/skills/` for `documentation-validation` or a collision name formed with the `documentation-validation` prefix and `-ai-data-compass` suffix, optionally followed by a numeric suffix such as `-2`. Follow that procedure's final validation and report unresolved issues. If neither a local procedure nor the skill exists, explicitly inform the user that no documentation-validation procedure is configured or installed, and require review.
